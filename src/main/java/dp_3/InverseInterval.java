...
public class InverseInterval<T extends Comparable<T>> implements FilterStrategy <T> {

    public InverseInterval() {
        this.hasInverseInterval = hasInverseBoundaries;
    }
    ...
    public List<T> filter(List<T> list, List<T> pattern) {
        ...
        for (T t : list) {
            if (acceptValue(t, lowerBound, upperBound) == false) {
                results.add(t);
            }
        }
        return results;
